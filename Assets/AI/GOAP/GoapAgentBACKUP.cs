...
//     // === NEW: Store interrupted action and world state ===
//     private GoapAction lastInterruptedAction; // Store last interrupted action
//     private HashSet<KeyValuePair<string, object>> interruptedWorldState;  // To store world state at interruption
...
//             // Save the world state at the time of interruption
//             interruptedWorldState = dataProvider.GetWorldState();
//         }
...
//                 // Pass the stored interrupted world state to the planner when resuming
//                 HashSet<KeyValuePair<string, object>> worldState = interruptedWorldState ?? dataProvider.GetWorldState(); // If no interrupted state, use the current state

//                 // Get the goal from the data provider
//                 HashSet<KeyValuePair<string, object>> goal = dataProvider.CreateGoalState();

//                 // Re-plan using the interrupted world state
//                 Queue<GoapAction> plan = planner.plan(gameObject, availableActions, worldState, goal);

//                 if (plan != null) {
//                     // If a valid plan is found, enqueue the interrupted action and continue
//                     currentActions = plan;
//                     lastInterruptedAction = null;  // Reset the interrupted action
//                     stateMachine.pushState(performActionState); // Resume performing action
//                 } else {
//                     Debug.Log("Replanning failed after interruption. Going back to idle state.");
//                     stateMachine.pushState(idleState); // Go back to idle state if re-planning fails
//                 }
